...
MODEL_PIPE_PATH = PUBLIC_RESULTS / "model_lr_splines_pipe.joblib"
MODEL_META_PATH = PUBLIC_RESULTS / "model_lr_splines_meta.joblib"
MODEL_REF_PATH = PUBLIC_RESULTS / "patsy_ref_df.joblib"
...
MONEY_VARS = {"act_fijo", "act_var", "cap_pen_ent", "yoprinm_pr", "ypenh", "ysubh"}
...
def prepare_raw_for_patsy(df_raw_like: pd.DataFrame, model_meta: dict[str, Any]) -> pd.DataFrame:
    df_tmp = df_raw_like.copy()

    cont_cols_model = list(model_meta.get("cont_cols_model", []))
    cont_cols_lineales = list(model_meta.get("cont_cols_lineales", []))
    bin_cols = list(model_meta.get("bin_cols", []))
    zero_inflated_vars = list(model_meta.get("zero_inflated_vars", []))
    uses_winsor_log1p = bool(model_meta.get("uses_winsor_log1p", False))
    winsor_limits = model_meta.get("winsor_limits")

    for v in cont_cols_model + cont_cols_lineales:
        if v in df_tmp.columns:
            df_tmp[v] = pd.to_numeric(df_tmp[v], errors="coerce")

    if uses_winsor_log1p:
        if winsor_limits is None:
            raise ValueError("El modelo indica winsor+log1p, pero no se encontraron winsor_limits en la metadata.")
        for v in zero_inflated_vars:
            if v in df_tmp.columns:
                vals = pd.to_numeric(df_tmp[v], errors="coerce").fillna(0.0)
                lo = winsor_limits[v]["p01"]
                hi = winsor_limits[v]["p99"]
                df_tmp[v] = np.log1p(vals.clip(lower=lo, upper=hi))

    for b in bin_cols:
        if b in df_tmp.columns:
            s = pd.to_numeric(df_tmp[b], errors="coerce").fillna(0.0)
            df_tmp[b] = (s > 0).astype(int)

    for v in zero_inflated_vars:
        if v in df_tmp.columns:
            pos_col = f"{v}_pos"
            s_raw = pd.to_numeric(df_raw_like[v], errors="coerce").fillna(0.0)
            df_tmp[pos_col] = (s_raw > 0).astype(int)

    return df_tmp


def build_X_from_raw_df_model(df_raw_like: pd.DataFrame, model_bundle: dict[str, Any]) -> pd.DataFrame:
    model_meta = model_bundle["meta"]
    design_info = model_bundle["design_info"]
    x_cols = list(model_meta["X_cols"])

    df_tmp = prepare_raw_for_patsy(df_raw_like, model_meta)

    Xm = patsy.build_design_matrices(
        [design_info],
        df_tmp,
        return_type="dataframe",
    )[0]

    Xm = Xm.drop(columns=["Intercept"], errors="ignore")
    Xm = Xm.reindex(columns=x_cols, fill_value=0.0)
    return Xm


def predict_bundle_on_raw(model_bundle: dict[str, Any], df_raw_like: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    Xm = build_X_from_raw_df_model(df_raw_like, model_bundle)
    pipe = model_bundle["pipe"]
    prob = pipe.predict_proba(Xm)[:, 1]

    if hasattr(pipe, "decision_function"):
        eta = pipe.decision_function(Xm)
    else:
        p_safe = np.clip(prob, 1e-6, 1 - 1e-6)
        eta = np.log(p_safe / (1 - p_safe))
...
def load_app_data(base_dir: Path) -> dict[str, Any]:
    ...
    try:
        pipe_path = base_dir / MODEL_PIPE_PATH.name
        meta_path = base_dir / MODEL_META_PATH.name
        ref_path = base_dir / MODEL_REF_PATH.name

        missing = [p.name for p in [pipe_path, meta_path, ref_path] if not p.exists()]
        if missing:
            data["model"] = None
            data["have_model"] = False
            data["model_error"] = (
                "Faltan archivos del modelo nuevo en public_results: " + ", ".join(missing)
            )
        else:
            pipe = joblib.load(pipe_path)
            model_meta = joblib.load(meta_path)
            patsy_ref_df = joblib.load(ref_path)

            if not isinstance(model_meta, dict):
                raise ValueError("La metadata del modelo no tiene el formato esperado (dict).")
            if not isinstance(patsy_ref_df, pd.DataFrame):
                raise ValueError("El archivo de referencia de patsy no es un DataFrame válido.")

            rhs_formula = model_meta["formula"].split("~", 1)[1].strip()
            ref_for_patsy = prepare_raw_for_patsy(patsy_ref_df.copy(), model_meta)
            design_info = patsy.dmatrix(rhs_formula, ref_for_patsy, return_type="dataframe").design_info
...
def evaluar_perfil_modelo(model_bundle: dict[str, Any], perfil: dict) -> tuple[float, float]:
    x_raw = prepare_input_row(perfil)
    p_arr, eta_arr, _ = predict_bundle_on_raw(model_bundle, x_raw)
    return float(p_arr[0]), float(eta_arr[0])
...
def comparar_escenarios_modelo(model_bundle: dict[str, Any], perfil_base: dict, escenarios: dict, grupo: str) -> pd.DataFrame:
    p_base, eta_base = evaluar_perfil_modelo(model_bundle, perfil_base)

    filas = []
    for nombre, perfil in escenarios.items():
        p, eta = evaluar_perfil_modelo(model_bundle, perfil)
        filas.append({
            "grupo": grupo,
            "escenario": nombre,
            "probabilidad": p,
...
app_ui = ui.page_navbar(
    ui.nav_panel(
        ...
        ui.card(
            ...
            ui.p(
                ...
                "sin exponer microdatos de la EFH."
            ),
            ui.p(
                "Los resultados descriptivos agregados presentados en esta app provienen de 71.703 registros imputados, "
                "mientras que la comparación principal entre modelos se apoya en 2.313 hogares únicos."
            ),
        ),
    ),
    ui.nav_panel(
    ...
    ui.nav_panel(
        ...
        ui.layout_sidebar(
            ui.sidebar(
                ui.p(
                    "Completa o ajusta las características del hogar para estimar la probabilidad de tenencia de vivienda propia.",
                    class_="helper-text",
                ),
            ...
            ui.row(
                ui.column(
                    ...
                    ui.div(
                        ui.card(
                            ui.card_header("Probabilidad estimada de tenencia vivienda propia"),
            ...
            ui.div(
                ...
                "Se usa como perfil base exactamente lo que la persona ingresa en la calculadora y se compara visualmente cómo cambia la probabilidad al modificar la edad y la bancarización. El efecto del ingreso principal se mantiene resumido en la tabla de escenarios.",
            ),
            ...
            ui.card(
                ui.card_header("Escenario 2: bancarización"),
...
def server(input: Inputs, output: Outputs, session: Session) -> None:
    ...
    def prediction_result() -> dict[str, Any]:
        ...
        try:
            p_arr, eta_arr, x_model = predict_bundle_on_raw(DATA["model"], x_in)
            p = float(p_arr[0])
    ...
    def input_row_table():
        ...
        df.columns = ["variable", "valor_ingresado_raw"]
